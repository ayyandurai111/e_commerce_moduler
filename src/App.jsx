...
const pdpProduct = {
  ...
  sku:              "MSN-WBL-NVY-42",
  category:         "Fashion",
  categoryPath:     [{ label:"Fashion", href:"#" }, { label:"Blazers & Suits", href:"#" }],
  badge:            "BESTSELLER",
  price:            389,
  originalPrice:    520,
  currency:         "$",
...
export default function App() {
  const [page, setPage] = useState("pdp"); // "home" | "plp" | "pdp"
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      {/* ── Dev page switcher ───────────────────────── */}
      <Box sx={{ position:"fixed", bottom:16, right:16, zIndex:9999, display:"flex", gap:1 }}>
        {[["home","Home","#1a1a2e"],["plp","Products","#3b82f6"],["pdp","PDP","#e94560"]].map(([id,label,color])=>(
      ...
      {page==="pdp"  && (
        <ProductDetailPage
          ...
          navbar={navbarProps}
          footer={footerProps}
          onAddToCart={({ product, quantity, selected }) => console.log("Add to cart:", product?.name, { quantity, selected })}
          onBuyNow={({ product, quantity, selected })    => console.log("Buy now:",    product?.name, { quantity, selected })}
